package com.bdna.automation.bdnapublish.dimview;

import org.apache.jmeter.protocol.java.sampler.JavaSamplerContext;
import org.apache.jmeter.samplers.SampleResult;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.bdna.automation.service.DimViewService;
import com.bdna.automation.template.BaseTestCase;

@Component
public class DIM_N_MD_HW_LIC_MPN_LPRICE_PO_VW extends BaseTestCase {
	private static final long serialVersionUID = 1L;
	@Autowired
	private DimViewService dimViewService;

	public SampleResult runTest(JavaSamplerContext context) {
		boolean result = false;
		SampleResult results = new SampleResult();
		try {
			setupValues(context);
			results.sampleStart();
			result = dimViewService.getCount(this.getClass().getSimpleName());
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			results.setSuccessful(result);
		}
		listParameters(context);
		return results;
	}
}